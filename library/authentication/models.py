...
class CustomUser(AbstractBaseUser):
    ...
    """
    first_name = models.CharField(max_length=FIRST_NAME_MAX_LENGTH, default=None)
    last_name = models.CharField(max_length=LAST_NAME_MAX_LENGTH, default=None)
    middle_name = models.CharField(max_length=MIDDLE_NAME_MAX_LENGTH, default=None)
    email = models.CharField(max_length=EMAIL_MAX_LENGTH, unique=True, default=None)
    ...
    def delete_by_id(user_id):
        ...
        return user_to_delete is not None
    ...
    def create(email, password, first_name=None, middle_name=None, last_name=None):
        ...
        """
        if len(first_name) <= FIRST_NAME_MAX_LENGTH and \
            len(middle_name) <= MIDDLE_NAME_MAX_LENGTH and \
            len(last_name) <= LAST_NAME_MAX_LENGTH and \
            len(email) <= EMAIL_MAX_LENGTH and \
            len(email.split('@')) == 2 and \
            len(CustomUser.objects.filter(email=email)) == 0:
    ...
    def update(self,
        ...
        if first_name != None and len(first_name) <= FIRST_NAME_MAX_LENGTH:
            user_to_update.first_name = first_name
        if last_name != None and len(last_name) <= LAST_NAME_MAX_LENGTH:
            user_to_update.last_name = last_name
        if middle_name != None and len(middle_name) <= MIDDLE_NAME_MAX_LENGTH:
